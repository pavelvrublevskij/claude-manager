const Usage = {
  ...
  renderSummary(data) {
    ...
    const c = data.cost;
    const items = [
      { label: 'Input Tokens', value: Usage.fmtTokens(t.input_tokens), sub: Usage.fmtCost(c.input), color: 'color-input',
      ...
      { label: 'Output Tokens', value: Usage.fmtTokens(t.output_tokens), sub: Usage.fmtCost(c.output), color: 'color-output',
        tip: 'Tokens generated by Claude in responses \u2014 text, code, tool calls, and thinking. The most expensive token type.' },
      { label: 'Cache Write', value: Usage.fmtTokens(t.cache_creation_input_tokens), sub: Usage.fmtCost(c.cache_write), color: 'color-cache-write',
      ...
      { label: 'Cache Read', value: Usage.fmtTokens(t.cache_read_input_tokens), sub: Usage.fmtCost(c.cache_read), color: 'color-cache-read',
        ...
        <table class="usage-table">
          ...
          <tbody>${Object.entries(byModel).map(([model, tokens]) => {
            ...
            return `<tr class="model-row${active}" onclick="Usage.setModel('${escapeHtml(model)}')" style="cursor:pointer">
              ...
              <td class="col-input">${Usage.fmtTokens(tokens.input_tokens)}</td>
              <td class="col-output">${Usage.fmtTokens(tokens.output_tokens)}</td>
              <td class="col-cache-write">${Usage.fmtTokens(tokens.cache_creation_input_tokens)}</td>
              <td class="col-cache-read">${Usage.fmtTokens(tokens.cache_read_input_tokens)}</td>
  ...
  renderPeriods(periods) {
    ...
    el.innerHTML = `<table class="usage-table">
      ...
      <tbody>${periods.map(p => `<tr>
        ...
        <td class="col-input">${Usage.fmtTokens(p.input_tokens)}</td>
        <td class="col-output">${Usage.fmtTokens(p.output_tokens)}</td>
        <td class="col-cache-write">${Usage.fmtTokens(p.cache_creation_input_tokens)}</td>
        <td class="col-cache-read">${Usage.fmtTokens(p.cache_read_input_tokens)}</td>
        <td class="col-cost">

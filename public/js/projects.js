  ...
  renderGrid() {
    ...
    grid.innerHTML = Projects.data.map(p => `
      <div class="card project-card" onclick="App.navigate('project-detail', { slug: '${p.slug}' })">
        ...
        <div class="project-path clickable-path" onclick="event.stopPropagation(); Projects.openFolder('${p.slug}')" title="Open folder in file explorer">${escapeHtml(p.path)}</div>
